...
const URL_INPUT = 'https://ai.jurepi.kr/ko/tools/hairstyle-recommendation';
const TITLE_INPUT = '헤어스타일 추천기';
...
describe('buildShareUrl', () => {
  ...
  it('percent-encodes reserved characters in url and title', () => {
    const tricky = buildShareUrl('x', {
      url: 'https://ai.jurepi.kr/ko/tools/a?b=1&c=2',
      title: 'A & B ? #tag',
    });
    const parsed = new URL(tricky);
    expect(parsed.searchParams.get('url')).toBe('https://ai.jurepi.kr/ko/tools/a?b=1&c=2');
