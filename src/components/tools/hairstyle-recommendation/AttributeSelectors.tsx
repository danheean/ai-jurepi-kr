      ...
      {pillGroups.map((group) => (
        <div key={group.key} className="space-y-2">
          <div className="flex items-center gap-2">
            <label className="text-caption-sm font-semibold text-mute block">
              {group.label}
            </label>
            {group.key === 'gender' && isGenderAutoDetected && values.gender && (
              <span className="text-caption-sm bg-surface-card text-mute px-2 py-0.5 rounded-full">
                {t('attr.genderAuto')}
              </span>
            )}
          </div>
            ...
            {group.options.map((option) => {
              ...
              return (
                <button
                  ...
                  className={`px-4 py-2 rounded-full text-button-md transition-all duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-focus-outer focus-visible:ring-offset-2 disabled:opacity-50 ${
                    isSelected
