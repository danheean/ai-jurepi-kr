...
description: ai.jurepi.kr의 DESIGN.md 디자인 시스템을 충실히 구현하는 방법. CSS 커스텀 프로퍼티 토큰(브랜드 + 액센트), 폰트 페어링, shadow 깊이, 도구별 액센트 정체성, 디자인된 hover/press/focus 상태, anti-template 규율, 접근성(WCAG 2.1 AA)·반응형. UI 컴포넌트·스타일·토큰·애니메이션·시각 구현 시 반드시 사용. (주의: ai.jurepi.kr은 Jurepi.kr의 형제 프로젝트, 일부 예시는 형제 프로젝트 참고 가능하나 각 도구의 액센트는 ai.jurepi.kr 스펙을 따른다.)
---

# Design System Fidelity — ai.jurepi.kr DESIGN.md 충실 구현
