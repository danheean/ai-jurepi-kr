---
name: ai-jurepi-tdd
description: ai.jurepi.kr에서 테스트 우선(TDD) 개발을 클린 아키텍처와 결합해 inside-out으로 진행하는 방법. RED→GREEN→REFACTOR 루프, 픽스처 JSON과 가짜 공급자로 결정적 도메인 테스트(실제 API 호출 금지), Zod 검증 테스트, Vitest 단위/컴포넌트 + Playwright E2E + axe 접근성, 커버리지 ≥80%(도메인 ≥90%). ai.jurepi.kr 기능·버그·리팩터링에서 코드보다 테스트를 먼저 쓸 때 반드시 사용.
---

# TDD Workflow — 클린 아키텍처용 inside-out TDD & AI 도구 테스트

이 스킬은 ai.jurepi.kr의 작업 규율이다. **테스트를 먼저 쓴다.** 이유는 두 가지: (1) AI 응답 검증·매핑처럼 *실패해야 할 경계 사례*는 테스트가 명세다(실제 API 호출 대신 픽스처 JSON 고장 케이스로 검증), (2) 클린 아키텍처의 안쪽 계층은 순수·결정적이라 테스트가 빠르고 신뢰할 수 있어 TDD의 마찰이 거의 없다.
...
### 실제 API 호출 금지, 픽스처 JSON + 가짜 공급자로 검증

도메인 테스트는 **실제 AI API를 절대 호출하지 않는다.** 대신:

1. **픽스처 JSON** — 실제 제공자 응답의 고장 케이스를 기록한 JSON 파일 (`src/__fixtures__/gemini-responses.json`). 성공·부분 실패·타입 오류·비구조적 응답 등을 포함.
2. **가짜 공급자** — 도메인의 `AIProvider` 포트를 구현하되, 픽스처에서 응답을 읽도록: `class FakeGeminiProvider implements AIProvider { async generateContent() { return fixtures.malformedResponse; } }`
3. **응답 매퍼 테스트** — 응답 검증/변환을 순수 함수로 단언: `parseAndValidateResponse(malformedJSON)` → `Result<Success, ValidationError>`.

```typescript
// src/domain/parseResponse.test.ts
test('malformed JSON응답은 ValidationError.JSON_PARSE 반환', () => {
  // Arrange
  const malformed = JSON.parse('{ invalid }'); // 실제로는 객체 아님
  // Act — 도메인이 정의한 응답 타입(Zod)로 검증
  const result = parseAndValidateResponse(malformed);
  // Assert
  expect(result.ok).toBe(false);
  expect(result.error.code).toBe('JSON_PARSE');
});

test('필수 필드 부재시 ValidationError.MISSING_FIELD', () => {
  const incomplete = { /* 'content' 필드 없음 */ };
  const result = parseAndValidateResponse(incomplete);
  expect(result.error.code).toBe('MISSING_FIELD');
  expect(result.error.field).toBe('content');
});

// 실제 유스케이스 + 가짜 공급자
test('공급자가 실패 응답을 내면 이를 타입화된 에러로 변환', async () => {
  const fakeProvider = new FakeGeminiProvider('error-response');
  const result = await processUserInput('번역해줘', fakeProvider);
  expect(result.ok).toBe(false);
  expect(result.error).toMatchObject({ code: 'AI_ERROR', message: expect.any(String) });
});
```

이를 통해 (a) 도메인은 제공자 구현(Gemini/OpenAI)을 몰라도 됨, (b) 검증을 실제 API 호출 없이 100% 테스트 커버하고, (c) 제공자 전환도 포트만 재구현하면 도메인·테스트 무변 — clean-architecture의 포트-어댑터 패턴의 핵심 검증법.

### 라우트 핸들러 테스트 — 가짜 공급자 주입, HTTP 경계 검증

```typescript
// src/app/api/tools/translate/route.test.ts
test('요청 body가 Zod 스키마 오류면 400 반환', async () => {
  const res = await POST(new Request('http://test', {
    method: 'POST',
    body: JSON.stringify({ /* 'text' 필드 누락 */ })
  }));
...
test('AI 공급자 오류는 503 반환', async () => {
  const fakeProvider = new FakeGeminiProvider('provider-error');
  const result = await translateHandler.execute('Hello', fakeProvider);
  // 또는 라우트가 fakeProvider를 의존성으로 받도록:
  const res = await POST(req, { provider: fakeProvider });
...
- **도메인이 반환하는 영어 기술 에러 문자열을 UI에 *원시 렌더*하면 /ko 누수다 — 로컬라이즈된 상태 요약(badge)이나 code→i18n 매핑만 표시한다.** jwt-decoder 실측: `verifySignature`가 `{verified:false, error:'Signature verification failed.'}` 같은 **영어 기술 메시지**(도메인은 로케일 무관이 옳다)를 반환하는데, `VerificationSection`이 이 `result.error`를 `<p>{result.error}</p>`로 원시 표시 → ko 페이지에 영어 노출. 도메인 에러는 개발자용 원문이고 UI는 로케일별이라는 경계가 무너진 것. **규율:** 도메인 에러는 UI에서 (a) 로컬라이즈된 상태 badge(`t('verification.failed')` 등 verified/failed/unsupported 3분류)로 요약하거나 (b) 도메인이 함께 반환하는 `code`를 `t('errors.<code>')`로 매핑해 표시한다. 원시 `error` 문자열은 화면에 렌더하지 않는다(콘솔/디버그용). 회귀 테스트로 **원시 영어 문자열이 DOM에 없음**을 단언(`queryByText('Signature verification failed.')` = null)해 누수 재발을 막는다. 이는 "미라벨 파생값은 실누수"의 에러 특화판이다.
...
- **오케스트레이터가 `useTranslations`를 *아예 안 import*하면 자식으로 넘기는·자체 렌더하는 모든 크롬(라벨·탭·버튼)이 하드코딩 영어가 되고, en 카탈로그로 렌더하는 유닛 테스트는 그 영어가 우연히 en 값과 일치해 전부 GREEN이다.** jwt-decoder 실측: `JwtDecoder` 오케스트레이터가 `useTranslations` 없이 탭 `Claims`/`Raw JSON`, 버튼 `Download`/`Copy Payload`를 리터럴로, `TokenInput`은 `label || 'Token'`, `ClaimsTable`은 헤더 `Value`/`Key`를 리터럴로 렌더 → /ko에 영어 6종 노출. tsc·jwt 유닛 221·전체 E2E·빌드·프리렌더 전부 그린, 리더 **ko 라이브 시각 게이트로만 적발**(이것은 "JS 계산 문자열 하드코딩"의 극단 변형: 컴포넌트가 `t()`를 *쓰다 틀림*이 아니라 *아예 안 씀* — 심지어 i18n 키[tabs.claims·download.button]가 카탈로그에 이미 존재하는데도 미사용). **예방:** ① 사람 대상 문자열을 렌더하는 컴포넌트는 `useTranslations` import를 강제(프레젠테이션 리프라도 라벨을 렌더하면 t() 소비). ② 리더는 통합 후 `grep -L "useTranslations" <사람대상-렌더 컴포넌트>`로 t() 미소비 컴포넌트를 색출 + /ko `main.innerText` 영어 스캔. ③ 컴포넌트 테스트를 en뿐 아니라 **`locale="ko"` + 실제 `ko.json`**으로도 최소 1개 렌더해 크롬 라벨이 한국어인지(영어 리터럴 부재) 단언하면 "en 값과 우연히 일치"하는 거짓 GREEN을 깬다.
...
- **트윈(짝) 컴포넌트는 짝으로 수정하고 짝으로 회귀 가드한다 — 한쪽만 고치면 동일 버그가 트윈에 생존한다.** cron-parser 실제: Unix `DescriptionText`와 Quartz `QuartzDescriptionText`가 동일한 "기본 템플릿에 ICU 플레이스홀더(`{n}`/`{day}`) 충전 + 이미 해석된 이름 직접 사용" 설계를 공유하는데, 1차 수정·테스트가 Unix만 커버해 Quartz 트윈의 everyNSeconds `{n}` 미충전·everyNMinutes 키 부재·와일드카드 dom 1..31 나열이 다음 audit까지 생존했다(전 유닛·tsc·빌드 그린). **예방:** 한쪽 컴포넌트/도메인을 고칠 때 `grep`으로 트윈(같은 계열 `*Description*`·`*Field*` 등)을 찾아 미러 수정 + **양쪽 모두**에 "미충전 `{…}`/MISSING_MESSAGE/원시 키패스 부재" 회귀 가드를 건다.
- **스타일만 단언하는 컴포넌트 테스트는 렌더 텍스트 결함을 은닉한다 — 클래스 존재가 아니라 렌더 문자열을 단언하라.** cron-parser 실제: 기존 `QuartzDescriptionText.test`의 everyNSeconds 케이스가 `expect(div).toHaveClass('bg-surface-sunken')`만 확인해, 본문이 `Every {n} seconds`(리터럴 브레이스)로 깨져 렌더돼도 통과했다. **예방:** 표시 컴포넌트 테스트는 `container.querySelector('p').textContent`에 대해 **①리터럴 브레이스 부재(`not.toContain('{')`) ②MISSING_MESSAGE 키패스 부재(`not.toContain('tools.')`/네임스페이스) ③실값 존재(`toContain('30')`)** 를 단언한다. 스타일 클래스 단언은 렌더 정합의 증거가 아니다.
