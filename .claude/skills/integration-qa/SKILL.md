---
name: integration-qa
description: ai.jurepi.kr의 통합 정합성을 경계면 교차 비교로 검증하는 방법. AI 공급자 경계(클라 요청↔서버 API 스키마↔응답 검증), 키 격리(서버 API 키만, 클라 번들 부재), 응답 타입 강제(Zod), 도구 i18n 키, Vitest 커버리지, Playwright E2E, axe 접근성, Lighthouse CWV, 시각 회귀. 모듈 완성 직후·통합 시점에 품질을 점진 검증할 때 반드시 사용.
---

# Integration QA — AI 도구 경계면 교차 비교 검증

대부분의 버그는 한 계층 *안*이 아니라 계층 *사이*에 산다. 각 모듈은 단독으로 그린이어도, 계약이 어긋나면 제품은 깨진다. AI 도구에선 **클라 요청 스키마 ↔ 라우트 핸들러 Zod 검증 ↔ 응답 스키마 ↔ 도메인 응답 매퍼 타입**의 네 경계가 특히 중요하다. 이 스킬은 그 틈을 *실행으로* 찾는다.
...
## ai.jurepi.kr 경계면 매트릭스 (교차 비교 대상)

| 경계 | A 쪽 | B 쪽 | 무엇을 비교 |
|------|------|------|-------------|
| 클라 ↔ 라우트 | React form component `name=""` | `src/app/api/tools/<tool>/route.ts` Zod `z.object({…})` 필드명 | 입력 필드명이 Zod 스키마와 정확히 일치(타입·필수 여부도) |
| 라우트 ↔ 응답 | Zod 스키마 출력 타입(`z.object({result:z.string()})`) | `parseAndValidateResponse()` 도메인 입력 타입 | 라우트가 보내는 응답 스키마가 도메인 파서가 기대하는 타입과 일치 |
| 도메인 ↔ UI 소비 | `translateTool.execute()` 반환 타입(`Result<TranslationOutput, TranslationError>`) | 컴포넌트/훅 `onResult(result)` 기대 파라미터 타입 | 도메인이 내보내는 타입과 UI가 받는 타입이 일치, 에러 코드 추적 가능 |
| 가짜공급자 ↔ 실제 | FakeGeminiProvider 반환 픽스처 응답 타입 | 실제 Gemini API 응답 타입(공식 문서) | 픽스처가 실제 API의 유효한 response shape인지 확인(또는 고장 케이스도 실적용) |
| 라우트 ↔ i18n | 라우트가 도메인에서 받은 에러 `code` | `t('errors.<code>')` i18n 메시지 키 | 모든 에러 코드가 양 언어 카탈로그에 i18n 키로 존재 |
| 레지스트리↔라우트 | `tools.filter(live)` | `/api/tools/<slug>` 라우트 생성 | 모든 live 도구에 API 핸들러 존재, 라우트명 오타 부재 |
| 레지스트리↔i18n | ToolMeta `id` 집합 | `tools.<id>.*` 메시지 키 + API 응답 라벨키 | 모든 live 도구에 ko/en 카피 존재(UI 라벨 + 에러 메시지) |
| i18n ko↔en | `messages/ko.json` 키 | `messages/en.json` 키 | 키 집합 동일(누락/오타 없음), API 응답 라벨 키 양쪽 존재 |
| 메타↔프리렌더 | 도구 SPEC의 SEO 요구(고유 title/description/JSON-LD 타입) | `out/<locale>/tools/<slug>.html` 실제 DOM | 도구별 고유 메타·유효 JSON-LD가 정적 HTML에 존재 |
| 서버 환경 ↔ 클라 | 서버 `process.env.AI_PROVIDER`/`GEMINI_API_KEY` | 클라 번들(`next/app` route handler import 제외) | 클라 번들에 `AI_PROVIDER`/API 키 누수 부재(grep 검증) |
