...
description: >-
  ai.jurepi.kr 도구를 검색엔진(SEO)과 생성엔진(GEO)에 동시에 노출시키는 도구별 발견성 플레이북.
  사람 방문자는 구글/네이버 검색으로, AI(ChatGPT·Perplexity·Gemini·Claude·AI Overviews)는 답변 인용으로 도구를 찾게 만든다.
  도구별 고유 메타(title/description/canonical/hreflang/OG), 구조화 데이터(schema.org JSON-LD: SoftwareApplication·WebApplication·FAQPage·HowTo),
  답변 우선(answer-first) 콘텐츠, `llms.txt`, AI 크롤러 robots 정책, 프리렌더(SSG) HTML 노출, CWV, 인용가능성(citability)을 다룬다.
  주의: api/* 라우트는 sitemap/robots에서 제외(search bot 크롤 금지), /tools/* 페이지만 색인.
...
# SEO · GEO 최적화 — ai.jurepi.kr 도구별 발견성 플레이북

ai.jurepi.kr의 성장 엔진은 **발견성**이다. 각 도구는 **두 관객**에게 닿아야 한다:
...
ai.jurepi.kr은 Cloudflare Workers 위 Next.js 앱(OpenNext)으로 배포된다. **필수 고려사항:**
- `/api/**` 라우트(AI 공급자 호출)는 sitemap/robots에서 명시 제외 — bot이 크롤하면 API 호출 비용 발생.
- `/tools/**` 페이지는 SSG 프리렌더 → 정적 HTML에 JSON-LD/메타 필수 포함.
- `llms.txt`/robots 정책도 `/api/**` 제외(다른 형제 Jurepi 프로젝트와 경로 일치).
